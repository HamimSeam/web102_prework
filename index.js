...
function filterUnfundedOnly() {
  ...
  const unfundedGames = GAMES_JSON.filter((song) => {
...
function filterFundedOnly() {
  ...
  const fundedGames = GAMES_JSON.filter((song) => {
    return song.pledged >= song.goal;
  });
...
const fundingExplanationStr = `A total of ${totalRaisedStr} has been raised for ${totalGames} ${
  totalGames == 1 ? "game" : "games"
}. As of now, ${numUnfundedGames} ${
  numUnfundedGames == 1 ? "game" : "games"
} remains unfunded. Please support us in funding these fun games!`;
